...
function musicListen() {
    time3 = setInterval(() => {
        if (musicOBJ.ended) {
            musicOBJ.currentTime = 0;// 添加自动循环播放
            musicStar();
...
function musicJump() {
    for (let i = 0; i < musicList.length; i++) {
        musicLast.onclick = function () {
            ...
            musicOBJ.src = "src/music/" + musicList[i] + ".mp3";
            musicBOXbc.src = "src/img/resource/music_img/" + musicList[i] + ".jpg";
            musicHead.src = "src/img/resource/music_img/" + musicList[i] + ".jpg";
            nowtime = 0;
            console.log(i);
            i--;
            if (i < 0 || i > 5) {
